...
Fush only manages SSH server information and scans available containers. It does not implement its own SSH or container execution protocols. Connections are established using the system's SSH client and/or docker exec commands generated by Fush.

SSH keys are also generated using ssh-keygen. Fush does not implement its own key-generation mechanism.
